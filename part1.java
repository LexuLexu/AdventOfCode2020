...
public class part1 {
	...
	public static ArrayList<Integer> createArrayList(String inputString) {
		
		ArrayList<Integer> intArray = new ArrayList<Integer>();
		
		for (String currentNum : inputString.split(",")) {
			intArray.add(Integer.parseInt(currentNum));
		}
	...
	public static int findResult(ArrayList<Integer> inputArray) {
		
		int result = 0;
		
		for (int a = 0; a < inputArray.size(); a++) {
			for (int b = 0; b < inputArray.size(); b++) {
				for (int c = 0; c < inputArray.size(); c++) {
					if (inputArray.get(a) + inputArray.get(b) + inputArray.get(c) == 2020) {
						result = inputArray.get(a) * inputArray.get(b) * inputArray.get(c);
					}
				}
			}
		}
